...
bool ImGui::ButtonWithHint(const char* label, const char* hint, const ImVec2& size_arg)
{
...
bool ImGui::CheckboxFill(const char* label, bool* v)
{
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(style.FramePadding.y * 8, style.FramePadding.y * 8));
...
#define IMGUI_scale(value, minFrom, maxFrom, minTo, maxTo)\
    ((maxTo - minTo) * (value - minFrom)) / (maxFrom - minFrom) + minTo

bool ImGui::SliderScalarFill(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ImGuiWindow* window = GetCurrentWindow(); 
    if (window->SkipItems)
        return false;
    ...
    const float w = ImMax(content_avail.x, 4.0f);
    ...
    // Render
    float fraction = ImSaturate(IMGUI_scale(*(float*)p_data, *(float*)p_min, *(float*)p_max, 0, 1));
...
bool ImGui::SliderFloatFill(const char* label, float* v, float v_min, float v_max, const char* format, float power)
{
    return SliderScalarFill(label, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
...
bool ImGui::SliderIntFill(const char* label, int* v, int v_min, int v_max, const char* format)
{
    return SliderScalarFill(label, ImGuiDataType_S32, v, &v_min, &v_max, format);
}
